    ...
    apps_data = [
        {
            ...
            "raw_summary": {},
            "metrics": [
                {"key": "acres", "name": "Area Covered (ac)", "value": 22.9},
            ],
        },
        {
            "application_type_key": "seeding",
            ...
            "metrics": [
                ...
                {"key": "skips", "name": "Skips (%)", "value": 0.45},
            ],
        },
    ]
    lines = _build_task_highlights(apps_data)
    assert len(lines) == 3
    assert "Population (seeds/ac): 34,146.76" in lines[0]
    assert "Singulation (%): 99.24" in lines[1]
    assert "Seed Hybrids: FS 6595X RIB, SV Rate" in lines[2]


def test_highlights_harvest_run():
    apps_data = [
        {
            "application_type_key": "global",
            "application_type_name": "Equipment-Wide",
            "raw_summary": {},
            "metrics": [
                {"key": "acres", "name": "Area Covered (ac)", "value": 22.53},
                {"key": "average_speed", "name": "Average Speed (mph)", "value": 3.39},
            ],
        },
        {
            "application_type_key": "harvest",
            ...
            "metrics": [
                ...
                {"key": "dryyieldavg", "name": "Average Dry Yield (bu/ac)", "value": 251.38},
            ],
        },
    ]
    lines = _build_task_highlights(apps_data)
    assert len(lines) == 4
    assert "Area Covered (ac): 22.53" in lines[0]
    assert "Average Speed (mph): 3.39" in lines[1]
    assert "Moisture (%): 20.56" in lines[2]
    assert "Average Dry Yield (bu/ac): 251.38" in lines[3]


def test_highlights_spray_run():
    apps_data = [
        {
            "application_type_key": "global",
            "application_type_name": "Equipment-Wide",
            "raw_summary": {},
            "metrics": [
                {"key": "acres", "name": "Area Covered (ac)", "value": 3.92},
            ],
        },
        {
            ...
            "raw_summary": {},
            "metrics": [
                {"key": "rate", "name": "Rate (gal/ac)", "value": 2.90},
            ],
        },
    ]
    lines = _build_task_highlights(apps_data)
    assert len(lines) == 1
    assert "Rate (gal/ac): 2.90" in lines[0]


def test_highlights_no_task_detected():
    apps_data = [
        {
            "application_type_key": "global",
            "application_type_name": "Equipment-Wide",
    ...
    assert _build_task_highlights(apps_data) == []


def test_highlights_appear_in_formatted_output():
    apps_data = [
        {
            "application_type_key": "global",
            "application_type_name": "Equipment-Wide",
            "application_name": "Equipment-Wide",
            "raw_summary": {},
            "metrics": [
                {"key": "acres", "name": "Area Covered (ac)", "value": 22.53},
                {"key": "average_speed", "name": "Average Speed (mph)", "value": 3.39},
    ...
    result = format_structured_summary(apps_data)
    # Highlights section appears before per-app sections.
    highlights_pos = result.index("Run Highlights")
    harvest_pos = result.index("Harvest")
