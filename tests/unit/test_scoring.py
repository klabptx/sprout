...
def test_confidence_good_quality_high_severity():
    ...
    c = compute_confidence(
        data_quality_flag=False, top_severity=0.9, severity_threshold=0.75
    )
...
def test_confidence_good_quality_low_severity():
    ...
    c = compute_confidence(
        data_quality_flag=False, top_severity=0.1, severity_threshold=0.75
    )
    from sprout.config import get_settings

    s = get_settings()
    expected = round(
        max(0.5, s.confidence_base_good - s.confidence_severity_penalty), 2
    )
    assert c == expected
...
def test_confidence_poor_quality_high_severity():
    ...
    c = compute_confidence(
        data_quality_flag=True, top_severity=0.9, severity_threshold=0.75
    )
    from sprout.config import get_settings
...
def test_confidence_poor_quality_low_severity():
    ...
    c = compute_confidence(
        data_quality_flag=True, top_severity=0.1, severity_threshold=0.75
    )
    from sprout.config import get_settings

    s = get_settings()
    expected = round(
        max(0.5, s.confidence_base_poor - s.confidence_severity_penalty), 2
    )
    assert c == expected
...
def test_confidence_minimum_floor():
    ...
    c = compute_confidence(
        data_quality_flag=True, top_severity=0.0, severity_threshold=1.0
    )
    assert c >= 0.5
